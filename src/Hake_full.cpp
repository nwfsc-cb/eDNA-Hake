...
Type objective_function<Type>::operator()()
{
  ...
  //PARAMETER(ln_sample_tau); // extra observation error for field samples. Additive on the log scale
...
// f = factorcombination, supplied from data (looks like 1,1,1,2,2,2,3,3,3, etc.)
  ...
  for(int y = 0; y < n_y; y++){
    ...
    for(int f=0; f < n_f;f++){ // loop over factors in each year
      ...
      tmp_mat.col(f) = matrix<Type>(vector<Type>(X_L(f) * bs_L(f,0)) + vector<Type>(Z_L(f) * beta_L)) ;
  ...
  D_i =  Xf*betaf + Xs*bs + eta_smooth_all  ;  // all fixed and smooth effects.
   ...
   for(int b = 0; b < n_bottle; b++){
      ...
      jnll -= dnorm(gamma_bottle(bottle_trim_idx(b)) , Type(0.), tau_bottle(bottle_trim_idx(b)),true);
      //jnll -= dnorm(gamma_bottle(bottle_trim_idx(b)) , Type(0.), exp(ln_tau_bottle_test),true);
   }
   // 
   // VECTOR VERSION WITHOUT THE IF STATEMENT. 
   //  jnll -= sum(dnorm(gamma_bottle , zero_bottle  , tau_bottle,true)); 

      jnll -= dnorm( ln_tau_bottle(0), Type(-2), Type(1),true);
 ...
 // D_i =  Xf*betaf + Xs*bs + eta_smooth_all  ;  // all fixed and smooth effects.
  ...
  //Kappa prior
  jnll -= dnorm(ln_kappa, Type(-4), Type(0.5),true);
    ...
    //Presence-Absence component of model.
     ...
     for(int i = 0; i < n_i; i++){ // likelihood
        ...
        // Extra variablity in field samples.  Prior to make this small (normal on the log scale)
          //jnll -= dnorm(ln_sample_tau,Type(-2),Type(1),true) ;
