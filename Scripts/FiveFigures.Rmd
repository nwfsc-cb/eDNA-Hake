...
mid.step %>% 
  ...
  tidyr::expand(taxon, rep) %>% #Now this can be right_joined with mid.step
...
color = rlang::enquo(color)
             ggplot(.x) +
             geom_point(aes(x = MDS1, y = MDS2, color = !!color), size = 3) +
             facet_wrap(~groups)  }
...
Analysis.by.prop %>% 
  mutate(CAPw = map2(spp.matrix, env, function(.x, .y){
    ...
    return(caps)}
                                ...
                                geom_point(aes(color = position, shape = groups), size = 3) + 
                                facet_wrap(~groups))
```
...
mid.step.morph %>% 
  group_by(eDNA.sample) %>% 
  tidyr::expand(new.morphotype, rep) %>% #Now this can be right_joined with mid.step
...
collapse_by_morph <- function(df, morphotype){
 morphotype = enquo(morphotype)
   df %>% 
     inner_join(conversion.df, by = "taxon") %>% 
  group_by(eDNA.sample, rep, !!morphotype) %>% 
  summarise(nReads = sum(nReads)) -> step1
   
   step1 %>% 
     group_by(eDNA.sample, rep) %>% 
     mutate(tot = sum(nReads)) %>% 
  group_by(eDNA.sample, rep,!!morphotype ) %>% 
  summarise(prop = nReads/tot) -> step2
   
   step2 %>% 
      group_by(eDNA.sample) %>% 
  tidyr::expand(eDNA.sample,!!morphotype, rep) %>% #Now this can be right_joined with mid.step
  right_join(step2,.) %>% # this should have ~ 800 NAs
  # group_by(is.na(prop)) %>% tally %>% # Nailed it
  mutate(prop = replace_na(prop, 0)) %>% 
  group_by(eDNA.sample,!!morphotype) %>% 
  summarise(prop = mean(prop))
   
}
species_by_rep %>% 
  collapse_by_morph(newest) -> morph.proportions.by.sample
...
morph.proportions.by.sample %>% 
  ...
  facet_grid(groups~new.morphotype) 
...
morph.proportions.by.sample %>% 
  left_join(metadata) %>% 
  group_by(new.morphotype, groups) %>% 
  nest() -> nester 
nester %>%   
pull(data) %>%
  set_names(nester$new.morphotype) %>%
...
nester %>% 
  pivot_wider(id_cols = new.morphotype, names_from = groups, values_from = c(data, weights)) %>% 
  ...
  select(new.morphotype, data, weights) -> nester2

nester2 %>% 
  mutate(plot = pmap(.l = list (a = new.morphotype, b = data, c = weights),
                     .f = function (a,b,c){
                       b %>% 
...
species.proportions.by.sample %>% 
  ...
  mutate(PERMANOVA.within.layer  = map2(bc, env, function(.x, .y){
           ...
           # setBlocks(perms) <- .y %>% pull(position)
          adonis2(formula = .x ~ groups,data = .y, permutations = perms) } ),
         PERMANOVA.within.layer =  set_names(PERMANOVA.within.layer, nm = position)) %>%
...
```{r}
Step2 <- read_csv(here("Data", "metadata_for_prop.csv"))
...
species.proportions.by.sample %>% 
  ...
  ungroup() %>% 
  filter (n> 10) %>% 
  semi_join(species.proportions.by.sample, .) -> species.for.plot

Step2 %>% 
  left_join(species.for.plot) %>% 
  ...
  group_by(taxon) %>% 
  nest() %>% 
  mutate(plot = map2(data, taxon, function(.x, .y){
  ggplot(.x) +
  ...
  geom_point (aes(y = depth, x = -lon, size = prop, color = groups)) +
  ...
  labs (y = "Depth", x= "longitude", title = .y)}
    )) -> with.plots

with.plots %>% pull(plot)
```



Same maps for groups

### Figure 5

How does the proportion of sequences from a particular morphotype, above and below the thermocline, in places with or without upwelling

```{r}
morph.proportions.by.sample %>% 
  left_join(metadata) %>% 
  mutate(above = case_when(position == "above" ~ "Above", 
                           TRUE                ~ "Below")) %>% 
  group_by(Station, groups, newest, above) %>%
  summarise (avg.prop  = mean (prop, na.rm = T)) %>% 
  # mutate(avg.prop = replace_na(avg.prop, 0)) %>% 
  pivot_wider(names_from = above, values_from = avg.prop, values_fill = 0) %>% 
  mutate(ratio = Above / (Above+ Below)) %>% 
  ggplot (aes(x = groups, y = ratio)) +
  geom_violin() +
  facet_wrap(~newest)
...
Average depth per species

```{r}
morph.proportions.by.sample %>% 
  left_join(metadata) %>% 
  group_by(Station, newest, groups) %>%
  summarise(weight.depth = weighted.mean(depth, prop)) %>% 
  ggplot(aes(y = weight.depth, x = groups)) + geom_boxplot() + facet_wrap(~newest)
