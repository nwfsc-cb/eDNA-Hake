...
distances <- function(tibble,Abundance = nReads ){
  ...
  tibble %>% 
   mutate( spp.matrix = map(data, ~.x %>% select(taxon, !!Abundance, eDNA.sample) %>%
                              pivot_wider(names_from =  taxon, values_from = !!Abundance, values_fill = 0) %>% 
                              column_to_rownames("eDNA.sample")),
           
           bc = map(data, ~tibble_to_matrix(.x, taxon = taxon, Abundance = !!Abundance, sample.name = eDNA.sample,
...
species.proportions.by.sample %>% 
  ...
  ungroup() %>% 
  nest() %>% 
  distances(Abundance = prop) -> Analysis.by.prop.no.outliers
