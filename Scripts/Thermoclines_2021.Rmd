...
```{r}
thermoclines2 %>% 
  slice (1:10) %>% unnest(data) %>% 
   select(Depth = `Depth (m)`, Temperature = `Temperature (degC)`, Salinity = `Salinity (psu)`, Chla = `Fluorescence (ug_per_l)`, O2 = `Oxygen (ml_per_l)`) %>% 
  pivot_longer (cols = -Depth, names_to = "Variable", values_to = "value") %>% 
  ggplot (aes (y = Depth, x = value), color = "red") +
  geom_line() +
    scale_y_reverse() +
