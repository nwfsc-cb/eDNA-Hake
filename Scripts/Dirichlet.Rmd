...
By.taxa.dataset %>% 
  ...
  group_by(eDNA.sample, rep,Transect, position, depth, collapsed) %>% 
  summarise (nReads = sum(nReads)) %>% 
  filter(!is.na(collapsed )) %>% 
  pivot_wider (names_from = collapsed, values_from = nReads, values_fill = 0) -> wide.biol

wide.biol %>% 
  pivot_longer(cols = c(-eDNA.sample,-Transect, -position, -depth, -rep),  names_to = "func.group", values_to = "nReads") %>% 
  write_csv(here("Data", "Data_for_STAN.csv"))
  #no tilapias
  left_join(metadata %>% 
