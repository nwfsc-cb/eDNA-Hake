...
What do we know about those that didn't have a final ID

```{r}
anti_join(ASV.table %>% group_by(Hash) %>% summarise(nr = sum(nReads)), Classification) %>%
  # distinct(Match) %>% separate(Match, into = c("Hash", "size"), sep = ";size=") %>%
  inner_join(Seqs) -> not.ided

BLAST_results <- read_table(here("Data", "Seqs.blasted.txt"),  col_names = c("Hash", "Match", "pident", "length", "mismatch", "gapopen", "qstart","qend" , "sstart", "send", "evalue", "bitscore", "taxid", "qlen"))


anti_join(not.ided, BLAST_results) %>% fasta.writer(df = ., sequence = Sequence, header = Hash, file.out = here("Data" ,"unclassified.fasta"))
...
new.BLAST %>% 
  ...
  pull() %>% 
  set_names(x = .,nm = .) %>% 
  get_lineage(worlds.taxonomy) -> newtaxids
  
Conversion.df <- tibble (taxid = names(newtaxids),
                         taxonomy = newtaxids) %>%
...
write_csv(Conversion.df, here("Data", "lineages_new_blast.csv"))


new.BLAST %>% 
  # distinct(taxid) %>%
  separate_rows("taxid",sep =  ";") %>%
  left_join(Conversion.df) %>% 
