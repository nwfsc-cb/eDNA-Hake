...
if(MODEL.TYPE=="lat.lon.smooth" | MODEL.TYPE=="lat.lon.smooth.base"){
  ...
  new_data <- new_data %>% rename(utm.lon=x,utm.lat=y,bathy.bottom.depth=depth_m)
  
  if(MODEL.TYPE=="lat.lon.smooth"){
    new_data_trim <- new_data %>% mutate(Y=rnorm(nrow(new_data))) %>% 
      dplyr::select(-lon,-lat,-Gridcell_ID,-WM_depth_m)
  }else if(MODEL.TYPE=="lat.lon.smooth.base"){
    new_data_trim <- new_data %>% mutate(Y=rnorm(nrow(new_data))) %>% 
  ...
  D_pred_summary <- data.frame(Mean=rowMeans(10^D_pred),
                               SD=apply(10^D_pred,1,sd),
                               Val=apply(10^D_pred,1,quantile,probs=PROBS) %>% t() %>% as.matrix())
  D_pred_log_summary <- data.frame(Mean=rowMeans(D_pred),
                                   SD=apply(D_pred,1,sd),
                                   Val=apply(D_pred,1,quantile,probs=PROBS) %>% t() %>% as.matrix())
  ...
  D_pred_smooth <- data.frame(Mean=rowMeans(D_smooth),
                              SD=apply(D_smooth,1,sd),
                              Val=apply(D_smooth,1,quantile,probs=PROBS) %>% t() %>% as.matrix())
  ...
  if(MODEL.TYPE=="lat.lon.smooth"){
    smooth.projections <- list(D_pred_combined,
                               D_pred_log_combined,
                               D_pred_bottom_depth_combined,
                               D_pred_smooth_combined)
  }else if(MODEL.TYPE=="lat.lon.smooth.base"){
    smooth.projections <- list(D_pred_combined,
                               D_pred_log_combined,
                               D_pred_smooth_combined)
