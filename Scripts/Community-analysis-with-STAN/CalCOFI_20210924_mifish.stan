...
model {
  { // Local variables declaration for making the Stan program less Memory hungry
    ...
    //log_eta_mifish ~ normal(log_eta_mifish_mean, eta_sigma);
    ...
    //target += normal_lpdf(log_eta_mifish_mean | log_eta_prior[1], log_eta_prior[2]) ; // - 1 * normal_lcdf(0 | log_eta_prior[1], log_eta_prior[2]);
