...
# fix some columns
dat.stand <- dat.stand %>% 
  dplyr::select(qPCR,well,sample,IPC_Ct,inhibition_rate,task,contains(SP)) %>% 
...
```{r}
dat.samp <- dat.all %>% 
  dplyr::select(qPCR,well,sample,IPC_Ct,inhibition_rate,task,Zymo,dilution,contains(SP)) %>%
...
# many-to-many here. seems like there are a few duplicate IDs?
# just keep track of this for now
dupe_IDs<-dat.id %>% add_count(sample)%>% filter(n==2)
...
dat.sfc <- dat.samp %>% 
  filter(depth_cat==0) %>% 
  mutate(is_inhibited=ifelse(inhibition_rate>INHIBIT.LIMIT,"Inhibited","Non-Inhibited")) %>% 
...
dat.sfc.p <- ggplot()+
  ...
  geom_sf(data=dat.sfc.sf,aes(color=copies_ul))+
...
dat.50.p <- ggplot()+
  geom_sf(data=coast,fill='gray50')+
  geom_sf(data=dat.50.sf,aes(color=copies_ul))+
  facet_wrap(~is_inhibited)+
  scale_color_viridis()
dat.50.p
```
...
dat.150.p <- ggplot()+
  geom_sf(data=coast,fill='gray50')+
  geom_sf(data=dat.150.sf,aes(color=copies_ul))+
  facet_wrap(~is_inhibited)+
  scale_color_viridis()
dat.150.p
```
...
dat.200.p <- ggplot()+
  geom_sf(data=coast,fill='gray50')+
  geom_sf(data=dat.200.sf,aes(color=copies_ul))+
  facet_wrap(~is_inhibited)+
  scale_color_viridis()
dat.200.p
```
...
dat.300.p <- ggplot()+
  geom_sf(data=coast,fill='gray50')+
  geom_sf(data=dat.300.sf,aes(color=copies_ul))+
  facet_wrap(~is_inhibited)+
  scale_color_viridis()
dat.300.p
```
...
dat.500.p <- ggplot()+
  geom_sf(data=coast,fill='gray50')+
  geom_sf(data=dat.500.sf,aes(color=copies_ul))+
  facet_wrap(~is_inhibited)+
  scale_color_viridis()
dat.500.p
```
...
plot_transect <- function(df,transect_id){
  ...
  outp <- dfsub %>% 
    ggplot(aes(transect_dist_km,depth_cat,size=copies_ul))+
    geom_point()+
    scale_y_reverse()+
    scale_x_reverse()+
    labs(x="Distance Along Transect",y="Depth",size="Copies (ul)",title=paste0("Transect ",transect_id))
  outp
}
```
...
cowplot::plot_grid(plotlist=transect.cross.sections[40:42],nrow=3)
```
## With Acoustic Backdrop
