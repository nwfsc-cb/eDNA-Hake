...
```{r}
CTD_profiles <- read_csv(here("Data", "1_converted","CTD_hake_data_10-2019.csv"))

CTD_profiles %>% 
  rename (CTD_Cast = `CTD Cast #`) %>% 
  semi_join(eDNA.samples, by = "CTD_Cast") %>% 
  filter (!is.na(CTD_Cast)) %>% 
  group_by(CTD_Cast ) %>% 
  summarise (mean.max.depth = mean(`EK-38kHz-DepthBelowSurface-M-VALUE`)) -> max.depth
```
