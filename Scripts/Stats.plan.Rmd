...
Joined.dataset
  ungroup() %>%
   # group_by(Transect) %>%
  nest() %>% 
  mutate( bc = map(data, ~tibble_to_matrix(.x, taxon = taxon,Abundance = Normalized.reads, sample.name = eDNA.sample, distance = "bray",transformation = "" )),
          jc = map(data, ~tibble_to_matrix(.x, taxon = taxon,Abundance = Normalized.reads, sample.name = eDNA.sample, distance = "bray",transformation = "pa" )),
         env = map (data, ~tibble_to_env(.x, taxon = taxon, Abundance = Normalized.reads, sample.name = eDNA.sample, everything()) ),
         ...
         PERMANOVA = map2(bc, env, function(.x, .y){adonis2(formula = .x ~ as.factor(Transect) +  position + groups,data = .y, permutations = 9999) }),
         ...
         # CAP_lat = map2(Wide, env, function(.x,.y){
         #   BiodiversityR::CAPdiscrim(formula = .x ~ Transect,dist = "bray",
         #                             data = .y,
         #                             permutations = 999)
         #   
         # })
...
By.taxa.dataset.metadata %>%
  ...
  mutate( bc = map(data, ~tibble_to_matrix(.x, taxon = taxon,Abundance = Normalized.reads, sample.name = eDNA.sample, distance = "bray",transformation = "" )),
         ...
         PERMANOVA = map2(bc, env, function(.x, .y){
           perm <- how(nperm = 9999)
           setBlocks(perm) <- with(.y, Transect)
           adonis2(formula = .x ~  position * groups,data = .y, permutations = perm) }),
         CAP_lat = map2(bc, env, function(.x,.y ){
