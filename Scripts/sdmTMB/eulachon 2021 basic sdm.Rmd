...
Here's the eulachon qPCR data derived from the 2021 hake survey. It has been processed in a separate script to append depth, location, and otehr covariate data.

```{r}
dat <- read_rds(here('Data','eulachon qPCR 2021 joined cleaned.rds'))
```
...
sfc.preds.plot <- ggplot() + 
  geom_raster(data=p, aes(x,y,fill = exp(est)))+
  geom_sf(data=coast,col=NA,fill='gray80')+
  scale_fill_viridis_b(breaks=c(0,1,2,5,100,500))+
  labs(x="",y="",fill="copies",title="Eulachon eDNA concentration, surface")+
  xlim(predbbox[1],predbbox[3])+ylim(predbbox[2],predbbox[4])
sfc.preds.plot
```
...
p50.preds.plot <- ggplot() + 
  ...
  geom_sf(data=coast,col=NA,fill='gray80')+
  scale_fill_viridis_b(breaks=c(0,1,2,5,100,500))+
  labs(x="",y="",fill="copies",title="Eulachon eDNA concentration, 50m")+
  xlim(predbbox[1],predbbox[3])+ylim(predbbox[2],predbbox[4])
p50.preds.plot
```
...
p150.preds.plot <- ggplot() + 
  ...
  geom_sf(data=coast,col=NA,fill='gray80')+
  scale_fill_viridis_b(breaks=c(0,1,2,5,100,500))+
  labs(x="",y="",fill="copies",title="Eulachon eDNA concentration, 150m")+
  xlim(predbbox[1],predbbox[3])+ylim(predbbox[2],predbbox[4])
p150.preds.plot
```