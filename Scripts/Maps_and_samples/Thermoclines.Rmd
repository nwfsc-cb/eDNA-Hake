...
all.CTDS.to.keep <- read_csv(here("Data","cleaned_CTD_stations.csv"))

files.inCTD <- list.files(path = here("Data","1_converted"), pattern = "*.csv")
 tibbles <- list.files(path = here("Data","1_converted"), pattern = "*.csv", full.names = T) %>% 
  purrr::map(read_csv)
# Are they all present
set_names(tibbles, files.inCTD)
...
all.CTDS.to.keep %>% 
  ...
  group_by(CTD_Cast) %>% 
  nest() %>%
  mutate(CTD_Data =purrr::map (CTD_Cast, ~ read_csv(file = glue::glue(here("Data","1_converted"),"/" ,.x, ".csv")))) -> all.CTDS.to.keep
...
base_map_hake +
  ...
  labs(fill = "depth")+
  theme(legend.position = "bottom") -> station.plot
...
sst.plot +station.plot+
  ggsave("../Plots and figures/plots.for.choosing.png", height=10,width = 10, dpi = 300 )
