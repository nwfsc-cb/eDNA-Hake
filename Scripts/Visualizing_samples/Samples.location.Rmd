...
eDNA.samples <- read_csv(here("Data","eDNA hake water samples.csv"))
...
CTD.stations <- read_csv(here("Data","CTD_hake_data_10-2019.csv"))
...
CTD.stations %>% 
  ...
  dplyr::select(Date,Time,Button,Station=`Station #`,lat,lon, CTD_Cast = `CTD Cast #`) %>% 
...
#  filter (Station %in% duplicated.entries) %>% 
  ...
  eDNA.samples %>% 
    left_join(list.of.stations.to.keep) %>% 
...
#   
  list.of.stations.to.keep <- read_csv(here("Data", "cleaned_CTD_stations.csv"))
```
# Make a Plot
...
ggplot(data=world) +
  ...
  ylab("Latitude") +
  theme_bw()+
  labs(fill = "depth")-> base_map_hake
save_rds(base_map_hake, here("Data", "Hake_map.rds"))
...
eDNA.samples %>% 
    left_join(list.of.stations.to.keep) %>% 
    distinct(Station, depth, lat, lon) %>% 
  mutate (depth  = case_when (depth == "sfc" ~ 0, 
                              TRUE  ~ as.numeric(depth))) -> depth.profiles
...
left_join( Ole.depth) %>% 
  filter (!is.na(depth))-> Sample.info
...
Step1 %>% 
  ...
  facet_wrap(~Transect, ncol = 2, scale = "free_x") +
  theme_minimal() +
...
ggsave(here("Presentations", "images", "depth.profile.png"), width =  10, height = 3 )
```
